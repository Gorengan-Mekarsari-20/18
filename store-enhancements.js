(() => {
  ...
  function trackedStockQuantity(product) {
  ...
  function stockAvailable(product) {
    const quantity = trackedStockQuantity(product);
    return product?.is_available !== false && (quantity === null || quantity > 0);
  }

  function stockLabel(product) {
    const quantity = trackedStockQuantity(product);
    if (!stockAvailable(product)) return "HABIS";
    return quantity === null ? "Tersedia" : `Sisa ${quantity}`;
  }

  function cartQuantity(id) {
    return Number(findItem(id)?.qty || 0);
  }

  function canAddStock(id, amount) {
    const product = LIVE_PRODUCTS[id];
    const quantity = trackedStockQuantity(product);
    return quantity === null || cartQuantity(id) + amount <= quantity;
  }

  function stockWarning(product) {
    const quantity = trackedStockQuantity(product);
    return quantity === null ? "Maaf, menu ini sedang habis." : `Stok ${product?.name || "menu"} tinggal ${quantity} porsi.`;
  }
  ...
  function updateCartWithLiveProducts() {
    ...
    cart = cart.filter(item => {
      ...
      const remaining = trackedStockQuantity(product);
  ...
  function applyProducts(rows) {
    ...
    currentRows.forEach(row => {
      ...
      STOCK[row.id] = stockAvailable(row);
  ...
  function applySettings(settings) {
    ...
    QRIS_IMAGE_DATA = String(settings.qris_image_data || "").trim();
    ...
    syncWhatsAppLinks();
    syncPaymentOptions();
    window.updatePaymentInfo?.("desktop");
    window.updatePaymentInfo?.("mobile");
    updateZoneOptions();
    updateOpenStatus();
  }
  ...
  function checkout(mode) {
    ...
    const unavailable = cart.find(item => !stockAvailable(LIVE_PRODUCTS[item.id]) || !canAddStock(item.id, 0));
    if (unavailable) return alert(`Stok ${unavailable.name} baru saja habis atau berubah. Silakan cek keranjang lagi.`);
  ...
  function enhancedShowReceipt(order) {
    ...
    const qris = onlinePaymentSelected(order.paymentMethod) && QRIS_IMAGE_DATA ? `<img class="qris-payment-image" src="${escapeHTML(QRIS_IMAGE_DATA)}" alt="Kode QRIS Gorengan Mekarsari">` : "";
    const paymentInfo = order.paymentInstructions || qris ? `<div class="receipt-payment"><strong>💳 Info pembayaran online</strong>${order.paymentInstructions ? `<br>${escapeMultiline(order.paymentInstructions)}` : ""}${qris}</div>` : "";
  ...
  function enhanceFunctions() {
    ...
    const oldChangeQty = window.changeQty;
    window.changeQty = function(id, delta) {
      const input = document.getElementById(`qty-${id}`);
      const pending = Math.max(0, Number(input?.value || 0));
      const product = LIVE_PRODUCTS[id];
      if (delta > 0 && product && !canAddStock(id, pending + 1)) {
        alert(stockWarning(product));
        return;
      }
      return oldChangeQty(id, delta);
    };
    const oldAddFromQty = window.addFromQty;
    window.addFromQty = function(id, name, price) {
      const product = LIVE_PRODUCTS[id];
      const pending = Math.max(0, Number(document.getElementById(`qty-${id}`)?.value || 0));
      if (product && !canAddStock(id, pending)) {
        alert(stockWarning(product));
        return;
      }
    ...
    window.addCombo = function() {
      ...
      if (product && !canAddStock("paketkomplit", 1)) return alert(stockWarning(product));
    ...
    ["updateItem", "removeItem", "clearCart"].forEach(name => {
      ...
      window[name] = function(...args) {
        if (name === "updateItem" && Number(args[1]) > 0) {
          const product = LIVE_PRODUCTS[args[0]];
          if (product && !canAddStock(args[0], Number(args[1]))) {
            alert(stockWarning(product));
            return;
          }
        }
    ...
    const oldUpdatePaymentInfo = window.updatePaymentInfo;
    window.updatePaymentInfo = function(mode) {
      oldUpdatePaymentInfo(mode);
      const isMobile = mode === "mobile";
      const suffix = isMobile ? "Mobile" : "Desktop";
      const select = document.getElementById(`paymentMethod${suffix}`);
      const help = document.getElementById(`paymentHelp${suffix}`);
      if (!select || !help || !onlinePaymentSelected(select.value) || !ONLINE_PAYMENT_ENABLED || !QRIS_IMAGE_DATA) return;
      const info = PAYMENT_INSTRUCTIONS || "Scan QRIS berikut, lalu kirim bukti pembayaran melalui WhatsApp.";
      help.innerHTML = `<strong>💳 Scan QRIS untuk membayar</strong><br>${escapeMultiline(info)}<img class="qris-payment-image" src="${escapeHTML(QRIS_IMAGE_DATA)}" alt="Kode QRIS Gorengan Mekarsari">`;
