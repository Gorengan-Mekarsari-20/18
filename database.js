(() => {
  ...
  const STOCK_APPLIED_STATUSES = new Set(["paid", "processing", "completed"]);
  ...
  function cleanProduct(product, id) {
    ...
    const hasStockQuantity = rawStock !== null && typeof rawStock !== "undefined" && String(rawStock).trim() !== "";
    const stockQuantity = hasStockQuantity ? Math.max(0, numberValue(rawStock, 0)) : null;
    return {
      id: cleanId,
      name,
      ...
      is_available: product?.is_available !== false && (stockQuantity === null || stockQuantity > 0),
  ...
  function statusConsumesStock(status) {
    return STOCK_APPLIED_STATUSES.has(normaliseStatus(status));
  }

  async function stockUpdatesForItems(items, direction) {
    const quantities = new Map();
    cleanItems(items).forEach(item => {
      if (!item.id) return;
      quantities.set(item.id, (quantities.get(item.id) || 0) + Math.max(1, numberValue(item.qty, 1)));
    });
    ...
    quantities.forEach((qty, id) => {
      ...
      const current = trackedStockQuantity(product);
      if (current === null) return;
      const next = Math.max(0, current + direction * qty);
      updates[`products/${id}/stock_quantity`] = next;
      updates[`products/${id}/is_available`] = next > 0;
    });
    return updates;
  }
  ...
  window.GM_DB = {
    ...
    async createSale(sale) {
      ...
      const status = normaliseStatus(sale?.status || "paid");
      const data = {
        ...
        items,
        status,
        stock_applied: statusConsumesStock(status),
        source: cleanText(sale?.source || "admin", 30) || "admin",
        created_at: new Date().toISOString(),
        created_by: session.user.uid
      };
      const stockUpdates = data.stock_applied ? await stockUpdatesForItems(items, -1) : {};
      await database.ref().update({ [`sales/${ref.key}`]: data, ...stockUpdates });
    ...
    async updateSale(id, changes) {
      ...
      const saleRef = database.ref(`sales/${id}`);
      const currentSnapshot = await saleRef.once("value");
      if (!currentSnapshot.exists()) throw new Error("Pesanan tidak ditemukan.");
      const current = currentSnapshot.val() || {};
      const update = { updated_at: new Date().toISOString() };
      const nextStatus = typeof changes?.status !== "undefined" ? normaliseStatus(changes.status) : normaliseStatus(current.status);
      if (typeof changes?.status !== "undefined") update.status = nextStatus;
      ...
      const shouldApply = statusConsumesStock(nextStatus);
      const stockUpdates = wasApplied === shouldApply ? {} : await stockUpdatesForItems(current.items, shouldApply ? -1 : 1);
      if (wasApplied !== shouldApply) update.stock_applied = shouldApply;
      const rootUpdates = { ...stockUpdates };
      Object.entries(update).forEach(([key, value]) => { rootUpdates[`sales/${id}/${key}`] = value; });
      await database.ref().update(rootUpdates);
      return (await saleRef.once("value")).val();
    },
    ...
    async removeSale(id) {
      ...
      const stockUpdates = sale.stock_applied ? await stockUpdatesForItems(sale.items, 1) : {};
      await database.ref().update({ [`sales/${id}`]: null, ...stockUpdates });
    },
