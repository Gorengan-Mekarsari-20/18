(() => {
  ...
  function clearNewProductForm() {
    ["new_name", "new_category", "new_price", "new_cost", "new_stock", "new_badge", "new_description", "new_image_url"].forEach(id => setField(id, ""));
  ...
  function renderSettings() {
    ...
    const qris = String(settings.qris_image_data || "").trim();
    const preview = $("paymentQrisPreview");
    if (preview) {
      preview.src = qris || "";
      preview.classList.toggle("hidden", !qris);
    }
    if ($("payment_qris_file")) $("payment_qris_file").value = "";
  }
        ...
        <div class="product-top">
          ...
          <div class="product-main">
            ...
            <div class="two-col">
              ...
              <div><label>Stok porsi</label><input class="p-stock" type="number" min="0" step="1" inputmode="numeric" value="${product.stock_quantity === null || typeof product.stock_quantity === "undefined" ? "" : numberValue(product.stock_quantity)}" placeholder="Kosong = tidak dihitung"><span class="help">Bila diisi, stok berkurang otomatis saat pesanan dibayar/diproses.</span></div>
  ...
  async function addProduct() {
    ...
    try {
      ...
      await GM_DB.createProduct({
        ...
        stock_quantity: fieldValue("new_stock"),
  ...
  async function saveProduct(card) {
    ...
    try {
      ...
      await GM_DB.updateProduct(id, {
        ...
        stock_quantity: card.querySelector(".p-stock").value,
        ...
        is_available: card.querySelector(".p-available").checked || (String(card.querySelector(".p-stock").value || "").trim() !== "" && Number(card.querySelector(".p-stock").value) > 0),
        image_data: imageData,
        image_url: imageUrl
      });
  ...
  async function chooseQrisPhoto(input) {
    const file = input?.files?.[0];
    if (!file) return;
    try {
      setMessage("adminMsg", "Memproses foto QRIS…");
      const imageData = await compressImage(file);
      state.pendingQrisImage = imageData;
      const preview = $("paymentQrisPreview");
      if (preview) {
        preview.src = imageData;
        preview.classList.remove("hidden");
      }
      setMessage("adminMsg", "Foto QRIS siap. Tekan “Simpan Pengaturan Toko”.", "ok");
    } catch (error) {
      input.value = "";
      setMessage("adminMsg", appError(error), "error");
    }
  ...
  function clearQrisPhoto() {
    state.pendingQrisImage = "";
    if ($("payment_qris_file")) $("payment_qris_file").value = "";
    const preview = $("paymentQrisPreview");
    if (preview) {
      preview.src = "";
    ...
    setMessage("adminMsg", "Foto QRIS akan dihapus setelah menekan “Simpan Pengaturan Toko”.", "ok");
  }
  ...
  async function saveSettings() {
    ...
    setBusy(button, true, "Menyimpan…");
    try {
      const qrisImage = typeof state.pendingQrisImage === "undefined"
        ? String(state.settings?.qris_image_data || "")
        : state.pendingQrisImage;
      await GM_DB.updateSettings({
        ...
        qris_image_data: qrisImage
      });
      state.pendingQrisImage = undefined;
      await loadAll({ quiet: true });
  ...
  function bindEvents() {
    ...
    $("payment_qris_file").addEventListener("change", event => chooseQrisPhoto(event.target));
    $("clearQrisBtn").addEventListener("click", clearQrisPhoto);
